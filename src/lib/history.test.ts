...
// uma cópia fresca por teste via resetModules + import dinâmico. O `flush`
// vem do mesmo registro de módulos: Solid 2 enfileira escritas de signal e
// só as expõe ao leitor depois de drenar a fila.
async function freshHistory() {
  vi.resetModules();
  const [mod, { flush }] = await Promise.all([import("~/lib/history"), import("solid-js")]);
  return { ...mod, flush };
}
...
describe("recordVisit", () => {
  it("começa vazio e registra uma visita com timestamp", async () => {
    ...
    const { flush, history, recordVisit } = await freshHistory();
